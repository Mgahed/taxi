    ...
    */
    ...
    public function register(DriverRegistrationRequest $request)
    {
        $mobileUuid = $request->input('uuid');
            ...
            $created_params = $request->only(['service_location_id', 'name','mobile','email','address','state','city','country','gender','car_make','car_model','car_color','car_number','vehicle_year','vehicle_type']);
...
// request has multi vehicles
    ...
    /*mail Template*/
        $user_name = $user->name;
        $mail_template = MailTemplate::where('mail_type', 'welcome_mail_driver')->whereActive(true)->first();
